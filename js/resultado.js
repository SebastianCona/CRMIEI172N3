...
function completarFormularioActualizar(element,index,arr){
 ...
 document.getElementById('txt_nombre_resultado').value = nombreResultado;
  ...
  function actualizarResultado() {
    // Obtenemos el nombre del resultado desde la interfaz
    var nombre_resultado = document.getElementById("txt_nombre_resultado").value;
    const myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");
    ...
    .then((response) => {
        ...
        if (response.status == 200) {
            ...
            errorAlert.innerHTML = 'No es posible actualizar el registro esta siendo utilizado.';
...
function eliminarResultado() {
  ...
  .then((response) => {
      ...
      if (response.status == 200) {
          ...
          errorAlert.className = 'alert alert-danger';
          errorAlert.role = 'alert';
          errorAlert.innerHTML = 'No es posible eliminar el registro, esta siendo utilizado';
          alertContainer.appendChild(errorAlert);
      }
  })
  .catch((error) => {
